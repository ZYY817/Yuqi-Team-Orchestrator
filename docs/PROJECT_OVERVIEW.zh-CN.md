...
> ⚠️ 当前为 Developer Preview，尚未全部真实验收通过。功能现状与后续方向见[🗺️ 功能实践情况](./FEATURES.zh-CN.md)。
...
| 🖥️ **Team UI** | 状态、依赖、模型、权限、Token、耗时、证据、变更文件、会话导航和可逆归档 |
...
> ⚠️ 每个存储目录只允许一个 Host 进程；测试与生产使用不同存储目录。完整说明见[📖 使用教程](./USER_GUIDE.zh-CN.md)。
...
- [🗺️ 功能实践情况](./FEATURES.zh-CN.md)（[English](./FEATURES.md)）
